...
def load_and_validate_features(path: str) -> pd.DataFrame:
    ...
    required = [
        "date", "year", "month", "risk",
        "humidity_streak", "rainfall_spike", "temp_stability",
...
def get_candidates(df: pd.DataFrame) -> pd.DataFrame:
    """
    Apply biological filter. Returns rows that are plausible outbreak peaks.
    This is a *necessary* condition for peak selection, not a sufficient one —
    the year-level literature annotation is the authoritative outbreak/no-outbreak
    decision.
    """
    mask = (
        (df["risk"] == 1) &
        (df["humidity_streak"] >= MIN_HUMIDITY_STREAK) &
        (df["rainfall_spike"] == True) &
        (df["temp_stability"] == True) &
        (df["month"].isin(OUTBREAK_SEASON_MONTHS))
    )
    cands = df[mask].copy()
    cands["intensity"] = cands[INTENSITY_COL_A] * cands[INTENSITY_COL_B]
    return cands
...
def validate_clean_year(year: int, year_candidates: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "warning": (
            "BIOLOGICAL FILTER FIRED ON CLEAN YEAR — verify annotation or "
            f"tighten filter (max intensity: {year_candidates['intensity'].max():.2f})"
...
def generate_gt():
    ...
    for split, (y0, y1) in [("Train", (2005,2016)),
                             ("Val",   (2017,2018)),
                             ("Test",  (2019,2021))]:
